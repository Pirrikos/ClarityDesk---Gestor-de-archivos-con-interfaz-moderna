...
def handle_drag_enter(tile, event: QDragEnterEvent) -> None:
        ...
        for url in mime_data.urls():
            ...
            if file_path and os.path.exists(file_path):
                if os.path.isdir(file_path):
                    ...
                    target_abs = os.path.abspath(tile._file_path)
...
def handle_drag_move(tile, event: QDragMoveEvent) -> None:
...
def handle_drop(tile, event: QDropEvent) -> None:
...
def _get_watcher(tile):
    """Get filesystem watcher from parent view."""
    if tile._parent_view and hasattr(tile._parent_view, '_tab_manager'):
        tab_manager = tile._parent_view._tab_manager
        if hasattr(tab_manager, 'get_watcher'):
            return tab_manager.get_watcher()
    return None
